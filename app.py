...
# 2. CUSTOM CSS (Jurus Paksa Tengah, Sejajar & Full Screen)
st.markdown("""
    <style>
    /* Background Utama */
    .stApp {
    ...
    /* Memaksa Judul & Subtitle Sejajar (Horizontal) */
    .header-container {
        display: flex;
        align-items: center;
    ...
    /* Kunci Sidebar di Awal (Sembunyikan tombol panah minimize) */
...
with st.sidebar:
    ...
    if uploaded_file:
        if st.button("🔄 Ganti File"):
            st.rerun()
    st.info("💡 Masukkan file laporan untuk memulai dashboard.")

# --- LOGIKA DASHBOARD ---
if uploaded_file:
    # JURUS FULL SCREEN: Sembunyikan Sidebar & Perlebar Konten
    st.markdown("""
        <style>
    ...
    try:
        ...
        fig.update_layout(template="plotly_dark", height=600, title=f"Visualisasi {pilihan}", title_x=0.5)
        st.plotly_chart(fig, use_container_width=True)

        st.markdown("---")

        # Tabel Hasil Konversi (Sekarang Selalu Muncul)
        ...
        if st.button("🚀 Buat Slide PowerPoint"):
            try:
                ...
                st.error(f"Fitur PPT membutuhkan Kaleido. Error: {e}")
